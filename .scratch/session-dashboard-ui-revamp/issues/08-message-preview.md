...
實際使用後發現本票原始設計有 bug：檔頭固定讀 20 筆一次性重複用於標題擷取與 `firstMessagePreview`，遇到 skill 呼叫等雜訊會把真實訊息擠出 20 筆窗口外，導致明明有內容卻顯示「無」（真實案例 `def4a233-...`）。修正為「找不到才逐步擴大檔頭讀取窗口（20→60→180→500）」，並讓 `lastMessagePreview`（僅此欄位，不含標題與 `firstMessagePreview`）改為接受使用者或 agent 任一方的最後一則真實訊息。詳見 deploy-log 2026-08-03T02:11:28Z 條目。

該修正上線後量測到兩個與規格預期落差較大的副作用，使用者確認後追加二次修正（見 deploy-log 2026-08-03T02:45:00Z 條目）：合法空 session（永遠找不到真實訊息）會被迫擴展讀到 500 筆上限才停，造成整體效能退化 60~70%——修正為讀到的筆數少於要求筆數（代表已達檔案結尾）就直接停止擴展；Codex 有高達 44% 的 session 因自動核准子流程而讓 `lastMessagePreview` 顯示原始 JSON——修正為 `isSyntheticCodexText` 新增 JSON 格式偵測，過濾掉這類回覆並往前找真正的對話內容。
